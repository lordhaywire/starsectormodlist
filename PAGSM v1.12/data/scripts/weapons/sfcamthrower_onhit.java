 ...
 */
public class sfcamthrower_onhit extends BaseCombatLayeredRenderingPlugin implements OnFireEffectPlugin,
        OnHitEffectPlugin,
        EveryFrameWeaponEffectPlugin {

    public sfcamthrower_onhit() {
    }

    protected List<sfcamthrower_onhit> trails;
    public void advance(float amount, CombatEngineAPI engine, WeaponAPI weapon) {
        if (trails == null) return;

        Iterator<sfcamthrower_onhit> iter = trails.iterator();
        while (iter.hasNext()) {
        ...
        if (weapon.getShip() != null) {
            ...
            for (sfcamthrower_onhit curr : trails) {
                if (curr.proj != null) {
        ...
        for (int i = 0; i < numIter; i++) {
            for (sfcamthrower_onhit trail : trails) {
...
//		Color inverted = new Color(255, 255, 100, 50);
        ...
        if ((float) Math.random() > 0.75f && target instanceof ShipAPI) {
            engine.applyDamage(
                    target,
                    point,
                    500,
...
//								   rampUp, dur, inverted);
    ...
    public void onFire(DamagingProjectileAPI projectile, WeaponAPI weapon, CombatEngineAPI engine) {
        ...
        sfcamthrower_onhit trail = new sfcamthrower_onhit(projectile, prev);
        ...
        if (trails == null) {
            trails = new ArrayList<sfcamthrower_onhit>();
        }
        trails.add(0, trail);
    }
...
//			offset.y += vel.y * 1f;
    ...
    public sfcamthrower_onhit(DamagingProjectileAPI proj, DamagingProjectileAPI prev) {
        this.proj = proj;
        this.prev = prev;
