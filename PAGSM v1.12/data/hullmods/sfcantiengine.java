...
public class sfcantiengine extends BaseHullMod {
	...
	private static final float ZERO_FLUX_MULT = 10f;
	private static final float ENGINE_MULT = 10f;
	private static final float SUPPLY_USE_MULT = 1.10f;
	private static final float SENSOR_MULT = 1.10f;
	private static final float SMOD_SPEED_BONUS = 10f;
	...
	public void addPostDescriptionSection(final TooltipMakerAPI tooltip, final ShipAPI.HullSize hullSize, final ShipAPI ship, final float width, final boolean isForModSpec) {
		...
		tooltip.addPara(sfc_antiengineText1, padList, Misc.getHighlightColor(),"10%");
	...
	public String getSModDescriptionParam(int index, HullSize hullSize) {
		if (index == 0) return "+" + (SMOD_SPEED_BONUS);
		if (index == 1) return "+" + (int) Math.round((1f - SMOD_TURN_BONUS) * 100f) + "%";
		return null;
	}
}
