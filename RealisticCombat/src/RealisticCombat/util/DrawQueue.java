...
import RealisticCombat.settings.Radar;
...
public final class DrawQueue {
    ...
    private final byte[] currentColor = new byte[]{ Byte.MAX_VALUE,
                                                    Byte.MAX_VALUE,
                                                    Byte.MAX_VALUE,
                                                    Byte.MAX_VALUE };
    ...
    public static void releaseDeadQueues() {
        ...
        for (Iterator<Map.Entry<WeakReference<DrawQueue>, IntBuffer>>
             iter = refs.entrySet().iterator(); iter.hasNext();) {
     ...
     * @since 2.0
     */
    public DrawQueue(final int initialCapacity) { this(initialCapacity, GL_DYNAMIC_DRAW); }
     ...
     * @since 2.0
     */
    public DrawQueue(final int initialCapacity, final int drawFlag) {
        // If using vertex buffer objects, allocate buffer space on the graphics card
        if (Radar.usesVertexBufferObjects()) {
    ...
    private void resize(final int newCapacity) {
        ...
        if (!finished) { vertexMap.flip(); colorMap.flip(); }
     ...
     * <p>
     * @param color    {@link Color} of the next shape. All vertices added until
     *                 this method is called again will use this color.
     * @param alphaMod {@code float} to multiply {@link Color}'s alpha channel by
     ...
     * @since 2.0
     */
    public void setNextColor(final Color color, final float alphaMod) {
     ...
     * @since 2.0
     */
    public void setNextColor(final float red,
                             final float green,
                             final float blue,
                             final float alpha)
    {
     ...
     * @since 2.0
     */
    public void setNextColor(final int red, final int green, final int blue, final int alpha) {
     ...
     * @since 2.0
     */
    public boolean addVertices(final float[] vertices) {
        // Ensure the vertex array has an even number of floats
        if ((vertices.length & 1) != 0) throw new RuntimeException("Vertices must be added in pairs!");
        ...
        for (int x = 0; x < vertices.length; x++) {
            ...
            if ((x & 1) == 0) for (byte b : currentColor) colorMap.put(b);
        }
        finished = false;
        return resized;
    }
     ...
     * @since 2.0
     */
    public boolean addVertices(final List<Vector2f> vertices) {
     ...
     * @since 2.0
     */
    public boolean addVertex(final float x, final float y) {
        // If this is a new set of data, clear out the old data first
        if (finished) clear();
        ...
        for (byte b : currentColor) colorMap.put(b);
     ...
     * @since 2.0
     */
    public boolean addVertex(final Vector2f vertex)
    {
     ...
     * @since 2.0
     */
    public void finishShape(final int shapeDrawMode) {
     ...
     */
    public void finish() {
        if (finished) throw new RuntimeException("DrawQueue is already finished!");
     ...
     */
    public void draw() {
        if (!finished) throw new RuntimeException("Must call finish() before drawing!");
