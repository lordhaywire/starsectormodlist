...
public final class SpriteBatch {
    ...
    public SpriteBatch(final SpriteAPI sprite) {
        this(sprite, GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
    }

    public SpriteBatch(final SpriteAPI sprite, final int blendSrc, final int blendDest) {
        ...
        offsetScaleX = (sprite.getCenterX() > 0f ? sprite.getCenterX() / (sprite.getWidth() * .5f)
                                                 : 1f);
        offsetScaleY = (sprite.getCenterY() > 0f ? sprite.getCenterY() / (sprite.getHeight() * .5f)
                                                 : 1f);
    }
    ...
    public void finish() {
        if (finished) throw new RuntimeException("SpriteBatch is already finished!");
        finished = true;
    }
    ...
    public void draw() {
        ...
        for (DrawCall call : toDraw) {
            ...
            glTranslatef((-call.width * 0.5f) * offsetScaleX,
                         (-call.height * 0.5f) * offsetScaleY, 0f);
