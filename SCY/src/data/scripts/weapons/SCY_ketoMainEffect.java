...
public class SCY_ketoMainEffect implements EveryFrameWeaponEffectPlugin , OnFireEffectPlugin{    
    ...
    public void advance(float amount, CombatEngineAPI engine, WeaponAPI weapon){
        ...
        if (!runOnce) {            
            ...
            if(SHIP.getOriginalOwner()==-1 || SHIP.getOwner()==-1){return;}
        ...
        } else if(!LDEATH && !SHIP.getFluxTracker().isOverloadedOrVenting() && !SHIP.getTravelDrive().isActive() && ASTRAPIOS.getCooldownRemaining()==0){
            //weapon warms-up if the ship isn't overloaded, venting, travelling in and out and if the weapon is ready to fire
            warmup=Math.min(1, warmup+amount/5);
            if(warmup>0 && !sound){
                sound=true;
                Global.getSoundPlayer().playSound("SCY_keto_load", 1, 1, ASTRAPIOS.getLocation(), SHIP.getVelocity());
            }
        } else {
            warmup=Math.max(-1f, warmup-amount*10);
            if(sound){
                sound=false;
                Global.getSoundPlayer().playSound("SCY_keto_unload", 1, 1, ASTRAPIOS.getLocation(), SHIP.getVelocity());
            }
        }
    ...
    public void onFire(DamagingProjectileAPI projectile, WeaponAPI weapon, CombatEngineAPI engine) {
        ...
        if(MagicRender.screenCheck(0.5f,weapon.getLocation())){
            ...
            for(int i=0; i<10; i++) {
                 engine.addHitParticle(
                        loc,
                        MathUtils.getRandomPointInCone(new Vector2f(0,0), 200, SHIP.getFacing()-45, SHIP.getFacing()+45),
...
//            );
    ...
    private void dot(float warmth, float charge, float amount){
        if(warmth+charge<=0){
            ...
            int frame=Math.round(warmth*(DOT.getAnimation().getNumFrames()-1));
            float color = Math.max(0,Math.min(warmth*4, 1));     
