...
public class SCY_sirenMainAnimation implements EveryFrameWeaponEffectPlugin, OnFireEffectPlugin{
    ...
    private boolean runOnce=false, hasFired=false, closed=true, open=false;
    
    private final String chargeID="E_CHARGE";
    private final String heatID="E_HEAT";
    private final String doorFLID="F_GUNLEFT";
    private final String doorFRID="F_GUNRIGHT";
    private final String doorRLID="G_GUNLEFT";
    private final String doorRRID="G_GUNRIGHT";
    ...
    public void advance (float amount, CombatEngineAPI engine, WeaponAPI weapon) {
        ...
        if(charge==1){            
            ...
            if(MagicRender.screenCheck(0.25f, weapon.getLocation())){
                ...
                MagicLensFlare.createSharpFlare(
                        ...
                        600,
                        0,
                        new Color(50,150,255,128),
                        Color.white
                );
            }
        }
        else if(charge==0){hasFired=false;}
        ...
        if(timer.intervalElapsed()){
            ...
            if(weapon.getChargeLevel()>0 && !hasFired){
                ...
                directionCheck=4;
                if(closed){                    
                    Global.getSoundPlayer().playSound("SCY_siren_open", 1, 1, SHIP.getLocation(), SHIP.getVelocity());
                    closed=false;
                }
            } else {
            ...
            if(directionCheck<=0){
                direction=-1;
                if(open){
                    Global.getSoundPlayer().playSound("SCY_siren_close", 1, 1, SHIP.getLocation(), SHIP.getVelocity());
                    open=false;
                }
            }
