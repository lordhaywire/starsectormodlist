...
public class SCY_phaseTorpedoAI implements MissileAIPlugin, GuidedMissileAI {
    ...
    */
    ...
    private final float DETONATION_RANGE=800;
    ...
    public void advance(float amount) {
        ...
        if(launch || timer>=check){
            ...
            //mine trigger
            if(MathUtils.isWithinRange(target, MISSILE, DETONATION_RANGE*ECCM/2)){
                engine.spawnProjectile(MISSILE.getSource(), MISSILE.getWeapon(), "SCY_phase_mine", MISSILE.getLocation(), MISSILE.getFacing(), new Vector2f(MISSILE.getVelocity()));
                engine.addHitParticle(MISSILE.getLocation(), MISSILE.getVelocity(), 50, 0.5f, 0.25f, Color.PINK);
                engine.addHitParticle(MISSILE.getLocation(), MISSILE.getVelocity(), 100, 1f, 0.1f, Color.WHITE);
                Global.getSoundPlayer().playSound("system_phase_cloak_deactivate", 1, 0.5f, MISSILE.getLocation(), MISSILE.getVelocity());
