...
public class Roider_TechExpeditionFleetAssignmentAI extends RouteFleetAssignmentAI {
	...
	protected void addLocalAssignment(RouteSegment segment, boolean justSpawned) {
		...
		if (pickSpecificEntity) {
			...
			if (target != null) {
				...
				fleet.addAssignment(FleetAssignment.PATROL_SYSTEM, target, days, "scanning " + target.getName().toLowerCase());
				return;
			} else {
				if (justSpawned) {
	...
	public void advance(float amount) {
        ...
        if (piracyCheck.intervalElapsed()) {
            ...
            if (fleet.isTransponderOn()) Misc.makeNotLowRepImpact(fleet, "roider_tOff");
            else Misc.makeLowRepImpact(fleet, "roider_tOff");
        }
	}
