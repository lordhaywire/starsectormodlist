 ...
 */
public class Roider_RoiderBaseIntelV2 extends BaseIntelPlugin implements EveryFrameScript, FleetEventListener,
																EconomyAPI.EconomyUpdateListener {
    public static void aliasAttributes(XStream x) {
        x.aliasAttribute(Roider_RoiderBaseIntelV2.class, "system", "s");
        x.aliasAttribute(Roider_RoiderBaseIntelV2.class, "market", "m");
        x.aliasAttribute(Roider_RoiderBaseIntelV2.class, "entity", "e");
        x.aliasAttribute(Roider_RoiderBaseIntelV2.class, "elapsedDays", "ed");
        x.aliasAttribute(Roider_RoiderBaseIntelV2.class, "duration", "d");
        x.aliasAttribute(Roider_RoiderBaseIntelV2.class, "bountyData", "b");
        x.aliasAttribute(Roider_RoiderBaseIntelV2.class, "tier", "t");
        x.aliasAttribute(Roider_RoiderBaseIntelV2.class, "matchedStationToTier", "mt");
        x.aliasAttribute(Roider_RoiderBaseIntelV2.class, "monthlyInterval", "i");

        x.alias("roider_baseBounty", BaseBountyData.class);
        BaseBountyData.aliasAttributes(x);
    }

    public static final String MARKET_PREFIX = "roider_fringeIndieBase_";
	...
	public static class BaseBountyData {
        public static void aliasAttributes(XStream x) {
            x.aliasAttribute(BaseBountyData.class, "bountyElapsedDays", "e");
            x.aliasAttribute(BaseBountyData.class, "bountyDuration", "d");
            x.aliasAttribute(BaseBountyData.class, "baseBounty", "b");
            x.aliasAttribute(BaseBountyData.class, "repChange", "r");
            x.aliasAttribute(BaseBountyData.class, "bountyFaction", "f");
        }
	...
	public static Logger log = Global.getLogger(Roider_RoiderBaseIntelV2.class);
	...
	public Roider_RoiderBaseIntelV2(StarSystemAPI system,
                String factionId, RoiderBaseTier tier) {
		...
		BaseThemeGenerator.AddedEntity added = BaseThemeGenerator.addNonSalvageEntity(system, loc, Entities.MAKESHIFT_STATION, factionId);
...
//		}
		...
		PortsideBarData.getInstance().addEvent(new Roider_RoiderBaseRumorBarEvent(this));
	}
    ...
    public void init() {
		...
		log.info(String.format("Added indie roider base in [%s], tier: %s", getSystem().getName(), tier.name()));
...
//		entity.getDetectedRangeMod().unmodify("gen");
	...
	protected void notifyEnding() {
		super.notifyEnding();
		log.info(String.format("Removing indie roider base at [%s]", getSystem().getName()));
			...
			if (reason == CampaignEventListener.FleetDespawnReason.DESTROYED_BY_BATTLE &&
				...
				if (battle.isPlayerInvolved()) {
					...
					if (payment > 0) {
						...
						result = new PersonBountyIntel.BountyResult(PersonBountyIntel.BountyResultType.END_PLAYER_BOUNTY, payment, rep);
					} else {
						result = new PersonBountyIntel.BountyResult(PersonBountyIntel.BountyResultType.END_PLAYER_NO_REWARD, 0, null);
					}
				}
			}

			boolean sendUpdate = DebugFlags.SEND_UPDATES_WHEN_NO_COMM ||
			 					 result.type != PersonBountyIntel.BountyResultType.END_OTHER ||
			 					 Global.getSector().getIntelManager().isPlayerInRangeOfCommRelay();
			sendUpdate = true;
			if (sendUpdate) {
...
//		String base = Misc.ucFirst(getFactionForUIColors().getPersonNamePrefix());
        String base = "Independent Roider";
