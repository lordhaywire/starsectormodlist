...
public class DAModPlugin extends BaseModPlugin {
    ...
    public void onGameLoad(boolean newGame) {
        if(!newGame){
            ...
            //check if the memkey exist
            if(!Global.getSector().getMemoryWithoutUpdate().contains(SPECIAL_FLEETS)){
                
                //check if the special ships are in the player fleet
                if(Global.getSector().getPlayerFleet().getMembersWithFightersCopy()!=null && !Global.getSector().getPlayerFleet().getMembersWithFightersCopy().isEmpty()){
                    for (FleetMemberAPI m : Global.getSector().getPlayerFleet().getMembersWithFightersCopy()){
                        if(m.getHullId().startsWith("diableavionics_virtuous") || m.getHullId().startsWith("diableavionics_IBB")){
                ...
                for (StarSystemAPI s : Global.getSector().getStarSystems()){
                    ...
                    //check if the special fleets exist
                    if(s.getFleets()!=null && !s.getFleets().isEmpty()){
                        for (CampaignFleetAPI f : s.getFleets()){
                            if (f.getName().equals(txt("gulfFleet"))||f.getName().equals(txt("virtuousFleet"))){
                                Global.getSector().getMemoryWithoutUpdate().set(SPECIAL_FLEETS, true);
                                return;
                            }
                            if(f.getFlagship().getHullId().startsWith("diableavionics_virtuous") || f.getFlagship().getHullId().startsWith("diableavionics_IBB")) {
                                Global.getSector().getMemoryWithoutUpdate().set(SPECIAL_FLEETS, true);
                                return;
                            }
                        }
                    ...
                    //check if the special ships are in storage
                    if(s.getAllEntities()==null || s.getAllEntities().isEmpty()) continue;
                    for (SectorEntityToken e : s.getAllEntities()){
                        if(e.getMarket()!=null){
                            if(e.getMarket().hasSubmarket(Submarkets.SUBMARKET_STORAGE) 
                                    && e.getMarket().getSubmarket(Submarkets.SUBMARKET_STORAGE).getCargo()!=null
                                    && e.getMarket().getSubmarket(Submarkets.SUBMARKET_STORAGE).getCargo().getFleetData()!=null
                                    && e.getMarket().getSubmarket(Submarkets.SUBMARKET_STORAGE).getCargo().getFleetData().getMembersListCopy()!=null
                                    && !e.getMarket().getSubmarket(Submarkets.SUBMARKET_STORAGE).getCargo().getFleetData().getMembersListCopy().isEmpty()
                                    ){
                                for (FleetMemberAPI m : e.getMarket().getSubmarket(Submarkets.SUBMARKET_STORAGE).getCargo().getFleetData().getMembersListCopy()){
                                    if(m.getHullId().startsWith("diableavionics_virtuous") || m.getHullId().startsWith("diableavionics_IBB")){
                                        Global.getSector().getMemoryWithoutUpdate().set(SPECIAL_FLEETS, true);
                                        return;
                                    }
                ...
                //couldn't find the ships, they were probably never spawned
                //SPAWN
