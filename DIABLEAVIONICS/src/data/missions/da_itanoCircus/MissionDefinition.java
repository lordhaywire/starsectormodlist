...
//        api.addObjective(minX + width * 0.25f, minY + height * 0.5f, "nav_buoy");   
    ...
    public class Plugin extends BaseEveryFrameCombatPlugin {        
        ...
        public void advance(float amount, List<InputEventAPI> events) {
            ...
            //fixed cam
            engine.getViewport().set(-(mapY*1.2f * screenX/screenY)/2, -1.2f*mapY/2, mapY*1.2f * screenX/screenY, mapY*1.2f);
            ...
            if(enemy==null && !engine.getFleetManager(FleetSide.ENEMY).getDeployedCopy().isEmpty()){
                ...
                enemy.getMutableStats().getMissileMaxSpeedBonus().modifyMult("itano", 2);
                enemy.getMutableStats().getMissileGuidance().modifyMult("itano", 2);
                enemy.getMutableStats().getMissileAccelerationBonus().modifyMult("itano", 1.5f);
                enemy.getMutableStats().getMissileMaxTurnRateBonus().modifyMult("itano", 2);
                enemy.getMutableStats().getMissileTurnAccelerationBonus().modifyMult("itano", 3);
                enemy.getMutableStats().getMissileWeaponRangeBonus().modifyMult("itano", 3);
                enemy.getMutableStats().getMissileHealthBonus().modifyMult("itano", 3);
            }
            ...
            engine.getFogOfWar(1).revealAroundPoint(enemy, 0, 2000, 0);
            ...
            if(!player.isAlive()){
                engine.addFloatingText(new Vector2f(), ""+clock, 200, Color.RED, null, .1f, .1f);
                
                engine.setDoNotEndCombat(false);
                engine.setCombatNotOverFor(5);
            }
        }
    }
}
